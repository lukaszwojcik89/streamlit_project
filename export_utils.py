...
from config import EXCEL_COLORS, EXCEL_COLUMN_WIDTHS
...
class ExcelStyles:
    ...
    header_alignment: Alignment = field(
        default_factory=lambda: Alignment(horizontal="center", vertical="center")
    )

    # Style danych
    data_font: Font = field(default_factory=lambda: Font(name="Calibri", size=10))
    data_alignment_left: Alignment = field(
        default_factory=lambda: Alignment(horizontal="left", vertical="center")
    )
    data_alignment_center: Alignment = field(
        default_factory=lambda: Alignment(horizontal="center", vertical="center")
    )
    data_alignment_right: Alignment = field(
        default_factory=lambda: Alignment(horizontal="right", vertical="center")
    )
    ...
    color_yellow: PatternFill = field(
        default_factory=lambda: PatternFill(
...
def format_worksheet_headers(
    worksheet: Worksheet, styles: ExcelStyles, column_count: int
) -> None:
    """
...
def format_worksheet_data(
    ...
    if numeric_cols is None:
        numeric_cols = {
            ...
            6: "0",
            8: "0.00",
        }  # Domyślne: Czas(h), %, Godz.twórcze(h)
...
def set_column_widths(worksheet: Worksheet, widths: Dict[str, int] = None) -> None:
    """
...
def export_to_excel(
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        worksheet.auto_filter.ref = (
            f"A1:{chr(65 + len(export_df.columns) - 1)}{len(export_df) + 1}"
        )

        # Arkusz 2: Podsumowanie (z formułami)
        summary_data = (
            df_summary.reset_index() if df_summary.index.name else df_summary.copy()
        )
...
def export_worklogs_to_excel(
    month_data: pd.DataFrame, selected_month: str, start_date: Any, end_date: Any
) -> tuple[io.BytesIO, str]:
    """
    ...
    end_str = (
        end_date.strftime("%d") if hasattr(end_date, "strftime") else str(end_date)
    )
