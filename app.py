...
def main():
    ...
    try:
        ...
        with st.spinner("📊 Agregुję dane..."):
            df_processed = aggregate_worklogs_to_report(df_worklogs)
        
        if df_processed.empty:
            st.error("❌ Nie udało się zagregować danych.")
            return
        
        st.success(f"✅ Załadowano {len(df_worklogs)} wpisów worklogs ({len(df_processed)} unikatowych zadań)")
